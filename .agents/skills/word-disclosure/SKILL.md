...
description: 为任意语言生成或更新 `word/<language>/` 目录下的 Obsidian Markdown 笔记。正文用中文写作，借文学或艺术作品揭示词语的得以生发场景。
---

# Word Disclosure

本 skills 由用户手动触发。输出格式、来源、分析、范文索引和自检都在 reference。诠释理论以海德格尔的中后期思想为内部锚，帮助分析避开主体—客体分离，不把词语机械理解为主体的属性、动作的附加成分或容器中的状态，不静态化理解空间、时间，不信任传统语法学的对单词成分的划界，一个单词的词性可能并不唯一。只锚定分析方向：除非词语或来源本身直接涉及海德格尔，正文不点名海德格尔，也不用哲学的专业术语。
...
## 边界
- 不编造引文、作品出处、译文出处、词源依据或最早来源。只有模糊关联时，就以猜测、揣摩的语气描述；找不到可验证来源时，就在最后的汇报中说“暂未找到可验证资料”。
